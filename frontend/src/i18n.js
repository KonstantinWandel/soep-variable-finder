...
export const STRINGS = {
  en: {
    ...
    'results.pipeline': 'Ranked by semantic similarity ({embedding}). Every result is a description published by the source itself; nothing on this page is generated text.',
    ...
    'crash.title': 'The page could not be rebuilt',
    'crash.body': 'Something in the saved search history could not be displayed. It is kept in this browser, not on the server, which is why clearing the cache does not help and a private window works. Clearing it here fixes it; nothing else is lost.',
    'crash.reset': 'Clear the saved history and reload',
    'results.filtersChanged': 'You changed the filters after this search, so the list below is the old one.',
    'results.reask': 'Search again with the new filters',
    'row.hintPortal': 'This link opens the entry page only. Search there for {name}.',
    'row.hintStatistic': 'This link opens the overview of the statistic. The tables with the numbers are one click further, under "Statistik (Tabellen)".',
    'row.copyTerm': 'Copy search term',
    'row.copied': 'copied',
  ...
  de: {
    ...
    'results.pipeline': 'Sortiert nach semantischer Ähnlichkeit ({embedding}). Jeder Treffer ist eine Beschreibung der Quelle selbst; auf dieser Seite steht kein automatisch erzeugter Text.',
    ...
    'crash.title': 'Die Seite ließ sich nicht aufbauen',
    'crash.body': 'Etwas am gespeicherten Suchverlauf ließ sich nicht darstellen. Er liegt in diesem Browser, nicht auf dem Server: deshalb hilft das Leeren des Caches nicht und im privaten Fenster geht es. Hier weggeräumt ist der Fehler behoben, es geht nichts weiter verloren.',
    'crash.reset': 'Gespeicherten Verlauf löschen und neu laden',
    'results.filtersChanged': 'Die Filter wurden nach dieser Suche geändert, die Liste unten ist noch die alte.',
    'results.reask': 'Erneut suchen, mit den neuen Filtern',
    'row.hintPortal': 'Dieser Link öffnet nur die Einstiegsseite. Dort noch einmal nach {name} suchen.',
    'row.hintStatistic': 'Dieser Link öffnet die Übersicht der Statistik. Die Tabellen mit den Zahlen liegen dort einen Klick weiter unter „Statistik (Tabellen)“.',
    'row.copyTerm': 'Suchbegriff kopieren',
    'row.copied': 'kopiert',
