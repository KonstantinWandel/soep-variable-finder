...
OPEN_ITEMS: Dict[str, Dict[str, str]] = {
    ...
    "zensus-2022": {"de": "1.440 Tabellen eingebunden, räumliche Ebene wird gerade je Tabelle aufgelöst",
        "state": "partial", "next": "1,440 tables from 12 statistics indexed with table-level links, confirmed by hand in a browser on 2026-08-25. The regional level is encoded in the opaque table code rather than the title; scripts/resolve_zensus_levels.py is resolving it per table through metadata/table and the autopilot folds the result in, after which the spatial filter reaches these records."},
